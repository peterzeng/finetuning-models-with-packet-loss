...
def plot_steps_vs_nodes(data, dataset_name, save_path, accuracy_threshold=0.75):
    ...
    ax.set_ylabel("Steps to Accuracy", fontsize=12)
    x = [2, 4, 6]
    ax.set_xticks(x)
    ax.set_xticklabels(["2", "4", "6"], fontsize=12)
    ...
    ax.legend(title="Packet Loss Rate", title_fontsize=12, fontsize=12)
    ax.grid()
...
def plot_steps_vs_loss_rate(data, dataset_name, save_path):
    ...
    ax.legend(title="Accuracy", title_fontsize=12, fontsize=12)
    ax.grid()
